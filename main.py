...
def lucky():
    if ( Whether_lucky!=False):
        try:
            ...
            data = res.read()
            data = json.loads(data)
            data = "爱情指数："+str(data["result"][1]["content"])+"   工作指数："+str(data["result"][2]["content"])+"\n今日概述："+str(data["result"][8]["content"])
            return data
        except:
...
def lizhi():
    if (Whether_lizhi!=False):
        try:
            ...
            data = json.loads(data)
            if data["code"] == 200:
                saying = data["result"]["saying"]
                transl = data["result"]["transl"]
                source = data["result"]["source"]
                return '[原]' + saying + ' --' + source+ '\n[译]' + transl;
            else :
                return "老婆老婆我爱你~ 就像老鼠爱大米！"
        except:
...
#推送信息
def send_message(to_user, access_token, city_name, weather, max_temperature, min_temperature, pipi, lizhi, alarm, tips, note_en, note_ch, health_tip, lucky_):
...
if __name__ == "__main__":
    ...
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, city, weather, max_temperature, min_temperature, pipi, lizhi,alarm,tips, note_en, note_ch, health_tip, lucky_)
    import time
    time_duration = 3.5
